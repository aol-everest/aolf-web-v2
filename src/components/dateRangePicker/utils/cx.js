...
export default function cx(props) {
  ...
  const isEndDate = endDate && day.format('YYYY-MM-DD') === endDate.format('YYYY-MM-DD');
  const isStartDate = startDate && day.format('YYYY-MM-DD') === startDate.format('YYYY-MM-DD');
  const active = isStartDate || isEndDate;
  const off = calendar.month() !== day.month() || dates.isNotBetween(minDate, maxDate, day);
